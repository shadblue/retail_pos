...
class MAINAPPLICATION():
    def __init__(self) -> None:
        ...
        style.configure("Treeview",font=myfontt,foreground='#2b2b2b',cellpadding=19)
        style.configure("Treeview.Heading",font=myfontt,foreground='#444',background='PowderBlue',cellpadding=19)
        ...
        style=Style()
        style.theme_use('clam')
        prodTree=Treeview(prodTreeCan,columns=("column1","column2","column3","column4","column5","column6"),show='headings',selectmode='browse')
        ...
        prodTree.column("column6",anchor=CENTER,width=80)
        
        prodTree.heading("#1",text="Product ID",anchor=CENTER)
        prodTree.heading("#2",text="Product Name",anchor=CENTER)
        prodTree.heading("#3",text="Company Name",anchor=CENTER)
        prodTree.heading("#4",text="Quantity",anchor=CENTER)
        prodTree.heading("#5",text="Price",anchor=CENTER)
        prodTree.heading("#6",text="Use",anchor=CENTER)
        ...
        style=Style()
        style.theme_use('clam')
        cusTree=Treeview(cusTreeCan,columns=("column1","column2","column3","column4","column5","column6"),show='headings',selectmode='browse')
        ...
        cusTree.column("column6",anchor=CENTER,width=80)
        
        cusTree.heading("#1",text="Product ID",anchor=CENTER)
        cusTree.heading("#2",text="Product Name",anchor=CENTER)
        cusTree.heading("#3",text="Company Name",anchor=CENTER)
        cusTree.heading("#4",text="Quantity",anchor=CENTER)
        cusTree.heading("#5",text="Price",anchor=CENTER)
        cusTree.heading("#6",text="Use",anchor=CENTER)
        ...
        itemTreeCan=Canvas(itemFrame,highlightthickness=0,bg='#ffffff')
        itemTreeCan.place(relx=0,rely=0,relheight=1,relwidth=1)
        # Creating a product tree
        
        itemTree=Treeview(itemTreeCan,columns=("column1","column2","column3","column4","column5"),show='headings',selectmode='browse')
        
        itemTree.column("column1",anchor=CENTER,width=80)
        itemTree.column("column2",anchor=CENTER,width=80)
        itemTree.column("column3",anchor=CENTER,width=80)
        itemTree.column("column4",anchor=CENTER,width=80)
        itemTree.column("column5",anchor=CENTER,width=80)
        
        
        
        itemTree.heading("#1",text="Product Name",anchor=CENTER)
        itemTree.heading("#2",text="Company Name",anchor=CENTER)
        itemTree.heading("#3",text="Quantity",anchor=CENTER)
        itemTree.heading("#4",text="Price",anchor=CENTER)
        itemTree.heading("#5",text="Use",anchor=CENTER)
        
        itemTree.place(relx=0,rely=0,relwidth=1,relheight=1)
        
        # alternating colors
        itemTree.tag_configure("gray",background="#eee")
        itemTree.tag_configure("normal",background="#ddd")
        
        
        
        # attaching scroll bars
        sb_vertical=Scrollbar(itemTreeCan,orient='vertical',command=itemTree.yview)
        sb_horizontal=Scrollbar(itemTreeCan,orient='horizontal',command=itemTree.xview)
        itemTree.configure(yscrollcommand=sb_vertical.set,xscrollcommand=sb_horizontal.set)
        sb_vertical.grid(row=0,column=1,sticky='ns')
        sb_horizontal.grid(row=1,column=0,sticky='ew')
        
        # configure position of grid for treeview
        itemTreeCan.grid_rowconfigure(0,weight=1)
        itemTreeCan.grid_columnconfigure(0,weight=1)
        ...
        style=Style()
        style.theme_use('clam')
        cartTree=Treeview(cartFrame,columns=("column1","column2","column3","column4","column5","column6"),show='headings',selectmode='browse')
        ...
        cartTree.column("column6",anchor=CENTER,width=80)
        
        cartTree.heading("#1",text="Product ID",anchor=CENTER)
        
        cartTree.heading("#2",text="Product Name",anchor=CENTER)
        cartTree.heading("#3",text="Quantity",anchor=CENTER)
        cartTree.heading("#4",text="Price",anchor=CENTER)
        cartTree.heading("#5",text="Total",anchor=CENTER)
        cartTree.heading("#6",text="Total",anchor=CENTER)
        ...
        def selectProd(event):
            ...
            prodNameEnt.insert(0,values[1])
            prodCompanyEnt.insert(0,values[2])
            prodQtyEnt.insert(0,values[3])
            prodPriceEnt.insert(0,values[4])
            prodUseEnt.insert(0,values[5])
        prodTree.bind("<ButtonRelease-1>",selectProd)
        
        def clrProd(event):
            if prodNameEntVar.get()=='' or prodCompanyEntVar.get()=='' or prodQtyEntVar.get()=='' or prodPriceEntVar.get()=='':
                response=messagebox.showerror("Error","The fields are already clear")
            else:
                # clear the fields
                prodNameEntVar.set('')
                prodCompanyEntVar.set('')
        ...
        def uptProd(event):
            ...
            n=prodNameEntVar.get()
            c=prodCompanyEntVar.get()
            q=prodQtyEntVar.get()
            p=prodPriceEntVar.get()
            u=prodUseEntVar.get()
            
            if prodNameEntVar.get()=='' or prodCompanyEntVar.get()=='' or prodQtyEntVar.get()=='' or prodPriceEntVar.get()=='':
                response=messagebox.showerror("Error","Please complete the required fields")
            else:
                prodTree.item(selected,values=(values[0],prodNameEntVar.get(),prodCompanyEntVar.get(),prodQtyEntVar.get(),prodPriceEntVar.get(),prodUseEntVar.get(),))
                
                curr.execute(
                    "UPDATE product SET product_name=%s,company=%s,quantity=%s,price=%s,product_use=%s WHERE id=%s",
                    (prodNameEntVar.get(),prodCompanyEntVar.get(),prodQtyEntVar.get(),prodPriceEntVar.get(),prodUseEntVar.get(),values[0]))
                
                conn.commit()
                
                response=messagebox.showinfo("Success",'Data updated succesfully')
                # clear the fields
                prodNameEntVar.set('')
                prodCompanyEntVar.set('')
                prodQtyEntVar.set('')
        ...
        updProdLbl.bind("<ButtonPress>",uptProd)
        ...
        def dltProd(event):
            ...
            pname=prodTree.item(selected)['values'][0]
            
            if prodNameEntVar.get()=='' or prodCompanyEntVar.get()=='' or prodQtyEntVar.get()=='' or prodPriceEntVar.get()=='':
                response=messagebox.showerror("Error","The fields are empty")
            else:
            
                del_query="DELETE FROM product WHERE product_name=%s"
                sel_data=(pname,)
                c.execute(del_query,sel_data)
                conn.commit()
                prodTree.delete(selected)
                closeAddProduct(event)
                response=messagebox.showinfo("success","Data deleted succesfully")
        dltProdLbl.bind("<ButtonPress>",dltProd)
        ...
        def displayitems():
            ...
            for data in fetch:
                if x % 2==0:
                    itemTree.insert('','end',values=data,tags=('even','row'))
                else:
                    itemTree.insert('','end',values=data,tags=("odd","row"))
                    x+=1
        displayitems()
        ...
        def selectItem(event):
            ...
            # grab record values
            data=itemTree.item(selected,'values')
            print("Selected items: ",data)
            print("First value of Selected items: ",data[0])
            ...
            x=1
            # x+=1
            cartTree.insert('','end',iid=x,values=(x,name,quantity,price,total,tot))
            ...
            # #     else:
            # #         my_tag='gray'
            # #     # cartTree.insert('','end',values=data,tags=(my_tag))
            # #     # cartTree.insert("",'end',iid=data[0],text=data[0],values=(data[0],data[1],data[2]),tags=('even','row'))
            #     if x % 2==0:
            # #         cartTree.insert(parent='',index='end',iid=x,text='',values=(data[0],data[1],data[2]),tags=('even',))
            #             cartTree.insert('','end',iid=x,values=(data))
            #     else:
            # #         cartTree.insert(parent='',index='end',iid=x,text='',values=(data[0],data[1],data[2]),tags=('odd',))
            # #         # cartTree.insert('','end',iid=data[0],text=data[0],values=(data[0],data[1],data[2]),tags=("odd","row"))
            #         cartTree.insert('','end',iid=x,values=(data))
            #         x+=1
            
            
        itemTree.bind("<ButtonRelease-1>",selectItem)
